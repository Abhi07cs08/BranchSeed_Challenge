...
def build_all_candidates(case, args):
    ...
    if args.rind_mm is None:                     # main() resolves this; we must too
        args.rind_mm = bs.auto_rind(case, False)
    labels, lab = bs.find_candidates(case, args.collar_mm, args.rind_mm, args.touch_mm,
                                     args.min_cand_voxels, False)
    if not args.no_split:                        # main() splits fused ostia; we must too
        labels, lab = bs.split_fused(case, labels, lab, args, False)
    items, alive = [], []
    for li in labels:
